...
| `MARIADB_NAME` | `db1` | mariadb, wordpress | WordPress database name |
| `MARIADB_USER` / `MARIADB_PWD` | `Oleg` / `test` | mariadb, wordpress, adminer (manually) | WordPress DB credentials, created by `mariadb.sh` |
| `WP_TITLE` | `Inception Wordpress Site` | wordpress | Site title passed to `wp core install` |
| `WP_PUBLIC_URL` | `https://oilyine.42.lu:8443` | wordpress | `home`/`siteurl` set after install (includes the forwarded port) |
| `WP_ADMIN_USER` / `WP_ADMIN_PWD` / `WP_ADMIN_MAIL` | `admin` / … / … | wordpress | WordPress administrator account created by `wp core install` |
| `WP_USER` / `WP_USER_PWD` / `WP_MAIL` | `oleg` / … / … | wordpress | A second, non-admin (`author`) WordPress user |
...
| `FTP_USER` / `FTP_PWD` | `ftpuser` / `test` | ftp | FTP account, chrooted to `/var/www/wordpress` |
| `KC_NAME` | `keycloak` | mariadb | Name of the second database created for Keycloak |
| `KC_ADMIN` / `KC_ADMIN_PWD` | `kcadmin` / … | keycloak (`KEYCLOAK_ADMIN`/`KEYCLOAK_ADMIN_PASSWORD`) | Keycloak bootstrap admin console credentials |
| `KC_DB_USER` / `KC_DB_PWD` | `kc_user` / … | mariadb, keycloak | Dedicated DB user/password for Keycloak's own database |
| `KC_REALM` | `Inception` | keycloak, wordpress | Realm created by `keycloak.sh`; must match the WP OIDC plugin config |
| `KC_CLIENT_ID` / `KC_CLIENT_SECRET` | `wordpress` / … | keycloak, wordpress | Confidential OIDC client used by the WordPress OpenID Connect plugin |
| `KC_TEST_USER` / `KC_TEST_USER_PWD` / `KC_TEST_USER_EMAIL` | `member` / … / … | keycloak | A ready-to-use end-user account for testing SSO login |
