...
**How AI was used:** this project's documentation — this README, `USER_DOC.md`, `DEV_DOC.md`, and the files under `docs/` — was written with Claude (Anthropic), by having it read the existing Dockerfiles, entrypoint scripts, `docker-compose.yml` and `.env` files and describe what they actually do, then cross-check the setup against the published 42 Inception evaluation criteria. Claude also applied a small set of concrete fixes flagged by that review directly to the infrastructure code: dropping a hardcoded `:8443` from internal URLs, renaming the volume host paths to `/home/<login>/data/...`, adding `image:` names matching each service, renaming the WordPress admin account so it no longer contains "admin", removing `srcs/.env` from version control in favor of a committed `srcs/.env.example` template, and fixing `srcs/Makefile`'s `down` target so it no longer silently deletes data volumes. Note: NGINX currently also serves plain HTTP on port 80 (redirecting to HTTPS) for local development convenience — this must be reverted to port 443 only before the actual evaluation; see [docs/EVALUATION.md](docs/EVALUATION.md). The Dockerfiles, entrypoint scripts, and Docker Compose stack architecture itself were designed and implemented by the student.
